...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (18a/T1-T2 complete; T3 READY at `efc01da`; T4 extraction-only verified at `b5ff7ee9cea0e78f4462a478bafa42f8f6e18805`; T4b READY at `5a19a816d4f6d90e7d3fbde80d39f562decd8c4b`; T5 and T6-T9 open) | Crucible/PS Task 2 requirements reviews ✅; later artifact/BOM/runtime gates remain | Custos Task 2 READY schema/toolkit contract -> PS Plan 54 immutable artifact/BOM -> Crucible Plan 88 StrategyRelease acceptance -> Custos Plan 19 verifier/runtime; no Speculum gate | Custos owns the execution ABI, artifact schema, toolkit and local verifier, not StrategyRelease or artifact selection. Historical T4 evidence remains immutable. T4b maps all 241 T4 target digests to typed targets, records 22 support files, and passes whole-package strict mypy at 0/40 base and 0/59 adapter. Its `READY_TYPING_CLOSURE` receipt permits only the T4b handoff; T5's public pre-import verifier contract and T6's immutable RC still block 18b and production-ready claims. The PS `build-image.sh` -> Crucible Python image lane remains independent compatibility only, never a team fallback or acceptance receipt. |
