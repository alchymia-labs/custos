...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 engine adapter PREPARED-BLOCKED; T6 portfolio semantics READY; T7A CR99 contract consumer READY; T7B/T8-T10 open) | PRE-PLAN `324da6e`; T4 one-store durability; T5 additive ready/terminal and bounded durable supervision; T6 canonical portfolio equity; T7A exact CR99 assets and strict parser/verifier; Plan 18 T5e and real CR99 main/0117/publication receipts remain gates | T5 real artifact capability -> T6 READY -> T7A READY -> T7B durable guard code -> T8a -> Plan 90A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance -> unchanged promotion/close-out | T5-T7A add no database/outbox. T7A is contract-only: CR99 branch is not on main, 0117 unexecuted and runtime publication false. Team daemon/live remain disabled. |
