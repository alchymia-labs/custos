...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (18a/T1-T2 complete; T3 READY at `efc01da`; T4 extraction-only verified at `b5ff7ee9cea0e78f4462a478bafa42f8f6e18805`; T4b typing closure, T5 and T6-T9 open) | Crucible/PS Task 2 requirements reviews ✅; later artifact/BOM/runtime gates remain | Custos Task 2 READY schema/toolkit contract -> PS Plan 54 immutable artifact/BOM -> Crucible Plan 88 StrategyRelease acceptance -> Custos Plan 19 verifier/runtime; no Speculum gate | Custos owns the execution ABI, artifact schema, toolkit and local verifier, not StrategyRelease or artifact selection. T4 clean exact-HEAD verification maps all 241 files one-to-one into base/adapter/private-vendor package namespaces with pinned-blob and fixed-input parity evidence; receipt is `VERIFIED_EXTRACTION_ONLY` and handoff remains false. Extracted source still carries an exact fail-closed baseline of 75 platform-neutral and 289 adapter typing errors; T4b must clear it before any whole-distribution strict or 18b production-ready claim. T5 verifier remains open. The PS `build-image.sh` -> Crucible Python image lane remains independent compatibility only, never a team fallback or acceptance receipt. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (19a T1 only until producer gates) | PRE-PLAN `324da6e`; Crucible Plan 89 migration 0116 + `CR89-0116-GENERATION-STORAGE`; Plan 99 migration 0117 + signed runner policy; Plan 18 staged/final BOM | 19a-c STOP -> 19d-T8a candidate -> Plan 90A compatibility -> locked runtime RC -> Plan 90B round trip -> PS Plan 56 exact-candidate acceptance -> same-digest promotion/close-out | Reuse one SQLite deep module for desired/applied/outcomes/outbox/cap reservations; add signed trust rotation/revocation/rollback, content-addressed atomic artifact activation, enqueue-failure freeze/quarantine, SQLite disaster recovery, locked Docker provenance and metrics/SLO/alert/runbook gates. No second journal or Plan 20. |
