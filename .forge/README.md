...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B native interception CODE-ONLY; T7C authenticated transport CODE-ONLY; T8a candidate + T8b Phase-A READY; T9-T10 open) | T7B schema v4 plus the public Nautilus execution-client factory facade close durable reservation and direct-submit interception code; daemon policy/store composition remains open. T7C production durable plus reconnect-denial attestation also remains open | daemon policy/store composition -> CR100 T8 runtime evidence -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Phase-A compatibility is consumed, but production daemon enforcement, transport evidence, exact runtime BOM/RC, Phase B, live, runtime and production readiness remain false. |
