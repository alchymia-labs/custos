...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 engine adapter PREPARED-BLOCKED; T6-T10 open) | PRE-PLAN `324da6e`; T4 one-store durability; T5 additive ready/terminal and bounded durable supervision; Plan 18 T5e real capability and Plan 99 policy remain gates | T5 real artifact capability -> T6/T7 -> T8a -> Plan 90A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance -> unchanged promotion/close-out | T5 adds no database/outbox. Restart budget lives in the existing lease; daemon fails on unexpected task exit. Team daemon/live stay disabled while artifact capability/policy gates are false. |
