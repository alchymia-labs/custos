...
**Step 3**: Minimal implementation
- **Extract to `src/custos/cli/_daemon.py`** (not `cli/main.py`, per C2 Option A): create a new module hosting `async def run_daemon(args: argparse.Namespace)` — the verbatim post-`_parse_args` half of legacy `cli/main.py:212-294` `_run()`. Move alongside it the relocated helpers `_build_vault` / `_build_host` / `_build_reconciler` / `_heartbeat_loop` (previously at `cli/main.py:132-209`). Rebuild `_build_vault` to construct **`PerKeyVault(vault_dir=args.vault_dir, tenant_id=args.tenant_id, initiator=args.runner_id)` unconditionally** — the C3 production replacement for the deleted `SopsAgeVault`. MockVault (`credential_vault.py:100-118` base mock class, `class CredentialVault(_BaseVault)`) is **NOT** wired into `_build_vault` runtime path — retained only for `pytest` fixtures. Dev/paper users must run `arx-runner vault put` before `arx-runner start` (CEO clean-break directive; see 偏离日志 DEVIATION row `_build_vault MockVault fallback disposition`). Legacy `cli/main.py:_run` (still present at T7 completion) becomes a one-liner: `asyncio.run(_daemon.run_daemon(_parse_args(argv)))`; T8 then rewrites `cli/main.py` to the 5-line clean-break stub and only `_daemon.py` survives.
...
| DEVIATION | `_build_vault` MockVault fallback disposition (T7 Step 3 / `src/custos/core/per_key_vault.py`) | MockVault runtime fallback removed. CEO decision (2026-07-10, option a): `_build_vault` unconditionally returns `PerKeyVault`; dev/paper users must create credentials via `arx-runner vault put` before `arx-runner start`. MockVault class (`credential_vault.py:100-118` base mock) retained only for pytest fixtures (unit-test scaffolding), never wired into runtime `_build_vault`. Rationale: aligns with CEO clean-break directive (line 71 + 89) — no soft-fallback bridge between old MockVault dev mode and new PerKeyVault production; single vault runtime path avoids lesson #35 boundary drift. Resolves R2 follow-up N5. | ✅ CEO wukai 2026-07-10 |
