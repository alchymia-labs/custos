...
| [17](plans/2026-07/17-vault-cli-json-format-symmetry-fix.md) | fix vault CLI JSON format symmetry | ⏳ In Progress | Plan 16 ✅ | downstream real Docker smoke failure on `arx-runner vault verify` | Unify public verify and runtime decrypt on an explicit JSON SOPS contract; replace mocked/low-level false-green coverage with a real public CLI roundtrip; retain local `custos-runner:v0.3.0` and defer remote publication. |
