...
| 实体 | 关键字段 | 不变量 |
|------|---------|-------|
| **DeploymentSpec**（本地缓存的云端权威）| `spec_id` · `generation`(从 1 开始、单调递增) · `trading_mode`(sandbox/testnet/live) · `lifecycle_state`(running/paused/stopped/archived) · `strategy_path` · `provenance_ref.credential_id` · `connector` · `pairs` · `leverage` · `strategy_config`(opaque dict) · `strategy_registry_name`(optional) · `code_hash`(optional; live 必须为 64 位小写 hex) · `sandbox.starting_balances` · `approved_by` · `risk_config` · `nautilus_config` | `custos.contracts.DeploymentSpec` 是 normative consumer model，未知字段拒绝；`spec_id` 与 `provenance_ref.credential_id` 必须匹配 `^[a-zA-Z0-9_-]{1,64}$`，分别保护 NATS status subject token 与 per-key Vault 路径；reconciler 在进入 G6/host 前先 `model_validate()`；sandbox 必须给 starting balances；live 必须给 strategy directory hash；`strategy_config` 原样传入 strategy factory；`strategy_registry_name` 若给出，loader 内省断言 name→class 绑定一致；`lifecycle_state=paused` 时 zombie watchdog 不升级 degraded |
| **RunnerRiskConfig**（`DeploymentSpec.risk_config` 内嵌）| `max_notional_per_runner` (str Decimal, USD 计价) · `fallback_breaker.max_notional` (str Decimal) · `fallback_breaker.max_drawdown_pct` (str Decimal, 百分比 `20`=20%) | wire 全 `str(Decimal)` 反 float 精度丢失（红线 0.4）；缺失字段回退结构性 floor default（non-live `200 USD`，live `1000 USD` per `local_cap.py` DEV-04-CAP-DEFAULT；`max_drawdown_pct=20%`）；live floor 判断只读 `trading_mode=live`，不把 lifecycle 当 trading mode；custos daemon 在 reconcile loop 内从 spec dict 现读，云端 spec 变更下轮生效 |
| **EngineStatus**（`ExecutionEngineProtocol.get_engine_status` 的 Tier-2 快照）| `phase`(running/degraded/unknown) · `position_count`(int) · `order_count`(int) · `open_notional` (Decimal) · `peak_equity` (Decimal, 引擎 host 追踪的 high-water mark) · `current_equity` (Decimal, `open_notional + Σ unrealized_pnl`) · `drawdown_pct` (Decimal, `(peak-current)/peak*100`) | 引擎侧计算（NT host 从 kernel.cache 读）；money 字段全 Decimal（构造时 `_reject_float_money` 拒 float，红线 0.4）；wire 序列化 `str(Decimal)`；reconciler `_breaker_tick` 消费 `current_equity` 供 fallback breaker drawdown 判定 |
| **DeploymentStatus**（本地生成，回报云端）| `status_id` · `spec_id` · `tenant_id` · `container_id` · `phase`(pending/running/degraded/stopped) · `observed_generation` · `health`(healthy/warning/error + reason) · `started_at` · `stopped_at` · `last_reconcile_at` · `reported_by_runner_id` · `reported_at` | `observed_generation` 单调（不倒退）；`reported_by_runner_id` 必等于本机；phase 变化必伴随 timestamp 更新；成功 reconcile 的 `stopped` / `archived` lifecycle 必上报 `phase=stopped`，其他成功 lifecycle 上报 `phase=running` |
...
- **严格 consumer validation**：`DeploymentSpec.model_validate()` 失败的消息不进入 G6、Vault 或 host
- **单调 generation**：generation 从 1 开始；`spec.generation > status.observed_generation` 触发 reconcile；`<` 说明云端回退，需人工介入
- **0.3.0 public seam**：producer 只通过 `custos.contracts.DeploymentMessage` / `arx-runner deployment publish` 交付 desired state；它不手写 NATS envelope、subject 或 engine 内部 hash。
- **下游所有权边界**：下游编排消费完整官方镜像，不能维护派生 Custos Dockerfile；下游只负责策略代码与 opaque `strategy_config` 的组装，runtime validation、stream topology、Vault 和 engine wiring 属于 Custos。
