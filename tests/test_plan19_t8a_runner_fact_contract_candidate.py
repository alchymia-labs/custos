"""Plan 19 T8a immutable RunnerFact producer candidate."""
...
INDEX_PATH = ROOT / "docs/authority/runner-fact-contract-candidate-assets-v1.json"
...
@pytest.mark.asyncio
async def test_candidate_is_emitted_by_the_production_outbox_and_continues_generation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    golden = _json(GOLDEN_PATH)
    sequence = _json(SEQUENCE_PATH)
    after = sequence["after_batch"]
    schema = _json(SCHEMA_PATH)
    ...
    second_input = [
        {key: value for key, value in fact.items() if key != "seq"} for fact in after["facts"]
    ]

    await outbox.enqueue(_authority(golden), identity, first_input)
    await outbox.enqueue(_authority(after), identity, second_input)
    pending = await outbox.pending()

    assert json.loads(pending[0].payload) == golden
    assert json.loads(pending[1].payload) == after
    assert pending[0].subject == pending[1].subject == sequence["subject"]
    assert pending[0].stream_key == pending[1].stream_key == sequence["stream_key"]
    ...
    assert after["capability_manifest_digest"] == sequence["after_capability_manifest_digest"]
    assert (
        after["capability_manifest_digest"]
        == hashlib.sha256(_canonical(sequence["after_capability_manifest"])).hexdigest()
    )
    assert sequence["generation_resets_sequence"] is False
    assert sequence["pending_signed_payload_rewrite_allowed"] is False
    assert sequence["legacy_cutover"]["pending_puback_drain_required"] is True
    assert sequence["active_generation_dispositions"] == {
