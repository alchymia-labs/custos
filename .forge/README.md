...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 engine adapter PREPARED-BLOCKED; T6 portfolio semantics READY; T7A contract consumer READY; T7B durable/guard CODE-ONLY; T8-T10 open) | PRE-PLAN `324da6e`; one RunnerFact SQLite DB/outbox; T7B schema v3 stores verified CR99 policy and removes DeploymentSpec risk authority; real CR99 main/0117/publication receipts remain gates | T5 real artifact capability -> T6 READY -> T7A contract -> T7B code-only -> native interception/reservation closure -> T8a -> Plan 90A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | T7B does not claim engine-boundary, daemon, live or runtime readiness. CR99 branch is not on main, 0117 is unexecuted and runtime publication is false. |
