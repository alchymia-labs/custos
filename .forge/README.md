...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (18a/T1-T2 complete; Task 2 receipt READY 2026-07-15; T3-T9 open) | Crucible/PS Task 2 requirements reviews ✅; later artifact/BOM/runtime gates remain | Custos Task 2 READY schema/toolkit contract -> PS Plan 54 immutable artifact/BOM -> Crucible Plan 88 StrategyRelease acceptance -> Custos Plan 19 verifier/runtime; no Speculum gate | Custos owns the execution ABI, artifact schema, toolkit and local verifier, not StrategyRelease or artifact selection. Runtime address is `deployment_instance_id`; contracts/core stay Python >=3.11, Nautilus distribution requires >=3.12. Task 2 READY proves only the contract handoff, not wheel/BOM/OIDC/runtime/Plan 88 completion or production readiness. The PS `build-image.sh` -> Crucible Python image lane remains independent compatibility only, never a team fallback or acceptance receipt. |
