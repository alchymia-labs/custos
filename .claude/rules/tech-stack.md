...
> **脚注 — `nautilus` extra 需 Python 3.12+**: base 包 (NoopHost / paper / 审计) 保持
> Python >=3.11 可装 (`requires-python` 不变); `nautilus` extra 引入的 NautilusTrader
...
生产 NT host 依赖 `nautilus-trader` 是 optional `nautilus` extra (需 Python 3.12+),
...
`nautilus` 是生产 host 依赖, `pip install custos-runner` 默认不装 (external audit
场景只读代码不跑 NT). 装法: `uv sync --extra nautilus` (需 3.12+ 解释器) 或
