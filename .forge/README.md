...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (`0.1.0rc2` retained as historical evidence; business-named V1 replacement RC required; T5e/T7-T9 blocked on PS/Crucible runtime receipts) | Current toolkit code, schemas, fields and statuses use business names and local gates pass; the changed bytes require protected publication and independent readback before PS/Crucible pins can be relocked | T5c -> replacement toolkit RC -> PS54 -> CR88 -> T5d-A -> CR89 -> T5d-B/Plan19 T2 -> T5e -> Plan19 T5; no Speculum gate | Custos owns execution ABI and runtime verification. Old rc2 bytes are not overwritten or accepted through a compatibility parser; capability/runtime/production remain false without the replacement and real external receipts. |
