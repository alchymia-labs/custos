...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B daemon boundary CODE-ONLY; T7C two-stage authenticated transport CODE-ONLY; T8a candidate + T8b Phase-A READY; T9-T10 open) | T7B schema v4, native client facade and daemon composition share the RunnerFact store and fail closed without owner policy. T7C now durably retains retiring credentials, consumes targeted superseded revocation and supports exact response-loss resubmission, but production durable and real reconnect-denial attestation remain open | CR99 publication/consumption -> CR100 T8 real runtime evidence -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Phase-A compatibility and T7C consumer code are ready, but real owner policy consumption, production transport evidence, exact runtime BOM/RC, Phase B, live, runtime and production readiness remain false. |
