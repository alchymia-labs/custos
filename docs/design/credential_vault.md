...
> 或解密接口；`decrypt` 只被本地 V1 credential resolver 调用，并绑定 signed
> credential scope 与当前 deployment instance。runner 出网只有 RunnerFacts，
> plaintext 永不上 NATS / HTTP。ARX 不读取 credential，Crucible 也只拥有 scope
> reference；credentials never leave the Custos host.
