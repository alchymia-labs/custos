...
> **Status**: 🔲 Todo (Phase 2 refined 2026-07-09; codex L1 peer HIGH-1/HIGH-2/MED-3/FU-2 directed-fix 2026-07-09 → APPROVED_WITH_FOLLOW_UPS, ready for execute-team)
...
|------|-------|--------|-----------|-------|
| T1.1 spike 路径 (a) 确认 (相 A 探索桥 + 相 B 生产 vendored) | 1 | 🔲 | | **HIGH-1**: 相 B gating 前置 T3.2 vendored; 相 A 临时 sys.path 非 gating; intra-slice 序 T3.2→T1.1 相 B |
| T1.2 registry-name 校验 + import-failure gate | 1 | 🔲 | | dict-key (非 Pydantic) |
| T2.1 supertrend config risk section (ps) | 2 | 🔲 | | CEO 参数 DP2; 前置 grep `_risk_controller =` 挂载点 |
| T2.2 ps RiskController 激活 test | 2 | 🔲 | | 红线 0.3 per-strategy 层 |
| T3.1 vendoring 决策 + provenance | 3 | 🔲 | | DP1 gate (方案 A) |
| T3.2 vendor supertrend 闭包 | 3 | 🔲 | | scout §4 最小 subset |
| T3.3 红线 0.4 float grep gate | 3 | 🔲 | | vendored 闭包审计 |
| T4.1 nautilus_config plumb | 4 | 🔲 | | 借 ps _create_node_config timeout |
| T5.1 e2e real supertrend sandbox | 5 | 🔲 | | 无 arx |
| T5.2 testnet paper→testnet e2e | 5 | 🔲 | | 生产化验收点; @integration |
| T6.1 sidecar/runner 退休 docs | 6 | 🔲 | | docs-only, DP3 |
| T6.2 close-out | 6 | 🔲 | | 强制末尾 |
...
- **完成日期**: {YYYY-MM-DD}
- **总 Task 数**: 12 (跨 6 Track, 含 close-out)
- **偏离数**: {N} (DEV-06-* 详见偏离日志; DP1/DP2/DP3 CEO 决策落地记录)
- **验证结果**: 全部通过 / 部分通过 (testnet e2e 可能 partial)
- **实施 commit 范围**: custos {first_sha}..{last_sha} + ps {first_sha}..{last_sha} (跨仓)
- **契约影响**: docs/domain.md (DeploymentSpec field list) + docs/design/nautilus_host.md (supertrend migration + toolkit) + docs/engines/nautilus.md + ps config.yaml
- **红线守护**: Non-Custodial 4 红线全数守住 (grep 记录, 含 vendored toolkit 红线 0.4 审计) — 见红线 gate 满足度表
- **失败模式覆盖**: NEW test (registry-mode / RiskController / toolkit provenance+float-audit / nautilus_config / e2e) + existing no-regression
- **遗留项**: per-runner cap (Plan 04) + ps shared 精选迁移/sync 纪律 (Plan 07) + arx web sidecar NATS-only 迁移 (arx follow-up) + testnet e2e 若 partial
