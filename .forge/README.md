...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (RC4 publication/readback succeeded but exposed plan-derived prerequisite filenames; sole V1 source is cleaned for RC5; T5e/T7-T9 remain blocked on real PS/Crucible runtime receipts) | Current toolkit code, schemas and Custos↔Crucible handoff are exact; RC5 must complete protected publication and independent readback before PS pins the toolkit authority | RC5 approval/readback -> PS54 -> CR88 C6 -> T5d-A -> CR89 -> T5d-B/Plan19 T2 -> T5e -> Plan19 T5; no Speculum gate | Custos owns execution ABI and runtime verification. Earlier RC bytes are immutable registry audit evidence only and have no compatibility parser; capability/runtime/production remain false without RC5 and real external receipts. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B daemon boundary CODE-ONLY; T7C corrected per-mode SIM/LIVE CODE-ONLY; T8a exact-subject candidate READY; T8b Phase-A REOPENED; T9-T10 open) | T7B schema v4, native client facade and daemon composition share the RunnerFact store and fail closed without owner policy. T7C now uses one supervisor with independent per-mode vaults, JWTs, connections, consumers, publishers and authority checks across isolated SIM/LIVE transport domains; production broker receipts remain open. | CR99 publication/consumption -> CR100 corrected T1 and real runtime evidence -> Custos per-mode consumer receipt -> corrected Plan 90 Phase A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Focused T7C and T8a code gates pass, but real owner policy consumption, CR100 producer/readback, explicit SIM/LIVE real-NATS evidence, corrected Phase A, exact runtime BOM/RC, Phase B, live, runtime and production readiness remain false. |
