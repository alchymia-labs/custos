...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + local runtime convergence | ⏳ In progress (19a T1 only until producer gates) | PRE-PLAN `324da6e`; Crucible Plan 89 command producer; Plan 90 pre-RC schema/golden receipt; Plan 99 runner-safety-policy-authority; Plan 18 staged candidate | 19a-d stop gates; Plan 90 post-RC real round trip; Plan 18 exact final; Custos 0.4.0; PS Plan 56 | Reuse one SQLite deep module for desired/applied/outcomes/outbox/cap reservations; commit every command outcome before ACK/TERM; fingerprint exact signed event bytes + subject; add generation to signed RunnerFact headers without resetting sequence; keep live fail closed until Plan 99 policy and non-bypassable enforcement land. |
