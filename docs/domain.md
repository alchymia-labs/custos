...
| 实体 | 关键字段 | 不变量 |
|------|---------|-------|
| **DeploymentSpec**（本地缓存的云端权威）| `spec_id` · `tenant_id` · `strategy_id` · `version` · `parameters`(JSON) · `execution_engine_binding_id` · `trading_mode`(testnet/sandbox/live) · `target_runner_id`（必等于本机 `runner_id`）· `generation`(单调递增) · `code_hash`(live 必持 provenance) · `pulled_at` · `strategy_registry_name`(optional) · `nautilus_config`(optional dict) · `risk_config`(optional dict, 见 `RunnerRiskConfig`) · `lifecycle_state`(paper/live/paused，watchdog paused 豁免) | 只读（云端权威）；`target_runner_id ≠ 本机 runner_id` 时忽略；`trading_mode=live` 时 `code_hash` 必与本地 image sha 匹配；`strategy_registry_name` 若给出, 加载后经 ps toolkit registry 内省断言 name→class 绑定与 loader 找到的类一致（防 heuristic 静默错加载）；`nautilus_config` 若给出, 支持 `timeout_{connection,reconciliation,portfolio,disconnection}` + `reconciliation_lookback_mins` 覆盖 NT 内部默认；`risk_config` 若给出, 承载 runner 层守护参数（cap + fallback breaker），字段全 `str(Decimal)`；`lifecycle_state=paused` 时 zombie watchdog 不升级 degraded |
| **RunnerRiskConfig**（`DeploymentSpec.risk_config` 内嵌）| `max_notional_per_runner` (str Decimal, USD 计价) · `fallback_breaker.max_notional` (str Decimal) · `fallback_breaker.max_drawdown_pct` (str Decimal, 百分比 `20`=20%) | wire 全 `str(Decimal)` 反 float 精度丢失（红线 0.4）；缺失字段回退结构性 floor default（`paper=200 USD`, `live=1000 USD` per `local_cap.py` DEV-04-CAP-DEFAULT；`max_drawdown_pct=20%`）；custos daemon 在启动 + reconcile loop 内均从 spec dict 现读，不缓存 → 云端 spec 变更下轮生效 |
