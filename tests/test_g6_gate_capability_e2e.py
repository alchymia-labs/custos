...
_check_g6_gate is exercised directly (not via the reconciler) so a single layer
...
from arx_runner.deployment_reconciler import _check_g6_gate
...
def test_ntlive_host_accepted_with_all_layers_passing(strategy_dir) -> None:
    ...
    _check_g6_gate(_LiveCapableHost(), _valid_spec(strategy_dir), _valid_credential())
...
def test_noophost_still_rejects_live(strategy_dir) -> None:
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="G6 gate"):
            _check_g6_gate(NoopHost(), _valid_spec(strategy_dir), _valid_credential())
...
def test_undeclared_capability_host_gets_structured_reject(strategy_dir) -> None:
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="G6 gate"):
            _check_g6_gate(_CapabilityLessHost(), _valid_spec(strategy_dir), _valid_credential())
    assert "g6_gate_live_capability_denied" in [e.get("event") for e in logs]
...
def test_layer1_capability_relaxed_double(strategy_dir) -> None:
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="G6 gate"):
            _check_g6_gate(host, _valid_spec(strategy_dir), _valid_credential())
    assert "g6_gate_live_capability_denied" in [e.get("event") for e in logs]
...
def test_layer2_venue_unsupported_relaxed_double(strategy_dir) -> None:
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="G6 gate"):
            _check_g6_gate(_LiveCapableHost(), spec, _valid_credential())
...
def test_layer4_credential_scope_violation_relaxed_double(strategy_dir) -> None:
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(RuntimeError, match="G6 gate"):
            _check_g6_gate(_LiveCapableHost(), _valid_spec(strategy_dir), bad_credential)
...
def test_layer4_skipped_when_no_credential(strategy_dir) -> None:
    ...
    _check_g6_gate(_LiveCapableHost(), _valid_spec(strategy_dir), None)
...
def test_non_live_mode_bypasses_all_layers(strategy_dir) -> None:
    ...
    _check_g6_gate(NoopHost(), spec, None)
