...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (revised 2026-07-14) | revised PS Plan 53; Crucible/PS ABI requirements review | Custos Task 2 schema/toolkit -> PS Plan 54 immutable artifact/BOM -> Crucible Plan 88 StrategyRelease acceptance -> Custos Plan 19 verifier/runtime; no Speculum gate | Custos owns the execution ABI, artifact schema, toolkit and local verifier, not StrategyRelease or artifact selection. Runtime address is `deployment_instance_id`; contracts/core stay Python >=3.11, Nautilus distribution requires >=3.12; production accepts signed wheels and proves zero-rewrite strategy migration. The PS `build-image.sh` -> Crucible Python image lane remains independent compatibility only, never a team fallback or acceptance receipt. |
