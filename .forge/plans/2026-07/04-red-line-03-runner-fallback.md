...
> **Status**: ✅ Completed (04a `3e85c50` + 04b `d0dd537` + 04b-fix squash landed; codex L1 REQUEST_CHANGES 1 HIGH + 2 MED closed via 04b-fix cycle; state snapshot publisher + WAL-backed path + risk_config live-refresh 三层 runtime-wire live; drawdown wire flip 确认真实; 见 `.forge/reviews/2026-07/04b-peer-codex.md` + 04b-fix commits)
...
> **✅ Amended 2026-07-10 (codex L1 fix cycle CLOSED)**: 原 close-out 声明的"完整 runtime-wire 兑现"部分, 由 codex L1 peer review (`.forge/reviews/2026-07/04b-peer-codex.md`) 发现 1 HIGH + 2 MED runtime-wire gap; 04b-fix cycle 已全部关闭:
> - **HIGH-1 CLOSED** (`b04071e`): `cli/main.py` 现导入 + 构造 `StateSnapshotPublisher` 并在 `--reconcile-strategy-id` 路径 `asyncio.create_task(publisher.run(stop, reconciler.active_spec_ids))`; `DeploymentReconciler.active_spec_ids()` 新公开方法; `test_main_starts_state_snapshot_publisher` + `test_reconciler_exposes_active_spec_ids` grep 实存
> - **MED-2 CLOSED (Option A)** (`b04071e`): publisher 从 `publish_fire_and_forget` (at-most-once) 切换到 `publish_telemetry_envelope` (JetStream + WAL stash on disconnect, at-least-once); `--wal-path` CLI flag 默认 `~/.custos/state/telemetry-wal.db`; `test_state_snapshot_wal_cached_when_disconnected` 直接用真 `ArxNatsClient` 断线路径实证 WAL stash
> - **MED-3 CLOSED (Option A)** (`1c9f3dd`): `RunnerNotionalCap.apply_config` + `FallbackBreaker.apply_config` 新增; `DeploymentReconciler._refresh_risk_config(spec)` 在每次 accepted spec 前先跑, cloud-side operator 编辑 `risk_config.max_notional_per_runner` / `fallback_breaker.max_notional` / `max_drawdown_pct` 下一 loop 生效; `risk_config_refreshed` structlog 事件只在值真变化时打 (no-op refresh 保持静默); 4 个 refresh 契约 test grep 实存
>
> **drawdown wire flip 仍是真的** (codex + safety-04b + team-lead 三方 grep 实证 `_breaker_tick` → `get_engine_status` → `FallbackBreaker.evaluate`)。红线 gate 表见下 — snapshot 层与 risk_config live 层从 `runtime_wire=partial` 升级到 `runtime_wire=live`。
...
| **0.3 失联 ≠ 停止 (per-runner layer)** | Cap: `test_cap_exceeded_during_disconnect_still_rejects` + `test_cap_is_live_guard_relaxed_double`. Zombie: `test_zombie_detection_works_when_arx_disconnected` + grace/blip/paused suite. Breaker (notional): `test_breaker_trips_during_arx_disconnect`. Breaker (drawdown): `test_breaker_trips_on_drawdown_during_arx_disconnect` (04b NEW — drawdown wire live-guard). Long-run: `test_arx_disconnect_long_run_guards_persist` (04b NEW — 60-tick persistence). State snapshot wire: `test_main_starts_state_snapshot_publisher` + `test_reconciler_exposes_active_spec_ids` + `test_state_snapshot_wal_cached_when_disconnected` (04b-fix NEW — HIGH-1 + MED-2). risk_config live-refresh: `test_local_cap_refreshes_when_spec_changes_max_notional` + `test_fallback_breaker_refreshes_when_spec_changes_max_notional` + `test_risk_config_refresh_is_noop_when_unchanged` + `test_risk_config_refresh_uses_lifecycle_for_live_floor` (04b-fix NEW — MED-3). | **Full runtime-wire**: `cli/main.py::_build_reconciler` constructs cap + breaker + watchdog (04a: `3e85c50`); reconciler `_breaker_tick` feeds `open_notional` + `current_equity` from Tier-2 `get_engine_status` into `FallbackBreaker.evaluate` (04b: `b5b3ef9`); watchdog tick runs per poll (04a); first-trip flatten idempotence via `was_frozen` gate (04b: `aec6a08`); **`StateSnapshotPublisher` scheduled via `asyncio.create_task(publisher.run(stop, reconciler.active_spec_ids))` alongside reconciler + heartbeat, riding the JetStream WAL-backed path (04b-fix HIGH-1 + MED-2: `b04071e`)**; **`DeploymentReconciler._refresh_risk_config(spec)` re-reads `spec.risk_config` on every accepted spec, `RunnerNotionalCap.apply_config` / `FallbackBreaker.apply_config` swap the enforced values, cloud-side operator edits take effect the next loop (04b-fix MED-3: `1c9f3dd`)**. | State snapshot arx-side consumer migration deferred (external to custos independent repo — arx-project follow-up). NT per-order intercept hook deferred to v1 pre-live (DEV-04a-CAP-ENFORCEMENT-HOOK-DEFER). | arx sidecar-to-NATS-snapshot migration (arx-project); v1 pre-live cap intercept plan |
| 0.4 Money math Decimal | `test_snapshot_dataclasses_reject_float_money` + `test_get_positions_returns_decimal_money` + `test_engine_status_drawdown_is_decimal` + `test_state_snapshot_publishes_str_decimal_money` + `test_local_cap_config_parses_decimal_not_float` + `test_breaker_drawdown_uses_decimal_not_float`. 04b-fix preserves the wire contract: `test_state_snapshot_publishes_str_decimal_money` still asserts every money field is a JSON string round-trippable through `Decimal(str(...))`. | All 04b money paths: `EngineStatus.peak_equity` / `current_equity` / `drawdown_pct` are host-tracked Decimal; NT-side impl `Decimal(str(...))` reconstruction (no float); state_snapshot wire serialises `str(Decimal)` via `NatsEnvelope.to_bytes()` (04b-fix moved transport into the client but the payload dict is identical); `_reject_float_money` runtime invariant guards every dataclass construction. Verification `grep -rnE 'float\(.*(price|amount|notional)' src/custos --exclude-dir=toolkit` = 0. | No defer. | — |

**兑现范围声明 (lesson #40)**: "Red-line 0.3 per-runner layer transitions from `code_coverage + partial runtime_wire` (04a: cap / zombie / notional breaker live; drawdown wire deferred) to **`code_coverage + full runtime_wire`** — the reconciler now feeds `current_equity` from Tier-2 `get_engine_status` into the fallback breaker so a peak-to-current drawdown breach flattens autonomously during an arx disconnect. **04b-fix cycle (2026-07-10)** closed the three codex L1 gaps that had been keeping the runtime-wire declaration partial: (a) `StateSnapshotPublisher` is now scheduled by the runner entrypoint alongside the reconciler + heartbeat and iterates active spec ids each interval, (b) the publisher rides `publish_telemetry_envelope` so a disconnected snapshot is WAL-stashed and replayed on the next connect, (c) `_refresh_risk_config` re-reads `spec.risk_config` on every accepted spec so cloud-side operator edits to cap / breaker ceilings take effect next loop. State-snapshot arx-side consumer migration is an arx-project follow-up per the single-repo self-sufficiency discipline; the custos-side push path is complete. NT per-order intercept hook (DEV-04a-CAP-ENFORCEMENT-HOOK-DEFER) remains a v1 pre-live follow-up — cap decision layer is runtime-wire, intercept layer is not."

### 04b-fix cycle summary (codex L1 REQUEST_CHANGES closure)

The three findings from `.forge/reviews/2026-07/04b-peer-codex.md` are all closed by the 04b-fix commits `b04071e` (HIGH-1 + MED-2) and `1c9f3dd` (MED-3). No new deviations were opened during the fix cycle; each finding accepted the recommended Option A resolution direction listed in the spawn packet:

- **HIGH-1** (`cli/main.py:224` — StateSnapshotPublisher never wired): `cli/main.py` imports `StateSnapshotPublisher`, constructs it after the host + reconciler are built when `--reconcile-strategy-id` is set, and starts it via `asyncio.create_task(publisher.run(stop, reconciler.active_spec_ids))`. `DeploymentReconciler.active_spec_ids()` is a new public method returning spec ids whose `container_id` is set — the composition root does not reach into private `_state`. The publisher `run` signature changed from `run(stop, spec_id: str)` to `run(stop, spec_id_source: Callable[[], Sequence[str]])` so N concurrent deployments produce N snapshots per interval without re-spawning tasks. Two new tests grep-实存: `test_reconciler_exposes_active_spec_ids` (unit) + `test_main_starts_state_snapshot_publisher` (integration-style with the runner path).
- **MED-2** (`state_snapshot.py:167` — at-most-once transport): switched the publisher from `publish_fire_and_forget` (drops on disconnect with only a structured log) to `publish_telemetry_envelope` (JetStream-backed, WAL-stashed on disconnect, replayed on next connect). The publisher builds a `NatsEnvelope` and hands it to the client — transport ownership moves to `ArxNatsClient`, publisher owns only the payload schema. `--wal-path` CLI flag defaults to `~/.custos/state/telemetry-wal.db`; `cli/main.py` mkdirs the parent so a fresh runner install has durable telemetry out of the box. New failure-mode test `test_state_snapshot_wal_cached_when_disconnected` directly exercises the real `ArxNatsClient` disconnected boot state and asserts the WAL row lands with the correct snapshot subject + payload shape (replacing the earlier `test_snapshot_cached_when_disconnected` which only tested the fire-and-forget no-op contract).
- **MED-3** (`cli/main.py:177` — RunnerRiskConfig docs-vs-code drift): `RunnerNotionalCap.apply_config` and `FallbackBreaker.apply_config` swap the enforced config atomically and return True when the value actually changed. The breaker's peak-equity high-water mark and frozen state are preserved across refresh — a cloud-side edit raises / lowers the limits without silently resetting the drawdown baseline or clearing an existing trip. `DeploymentReconciler._refresh_risk_config(spec)` runs on every accepted spec before the generation gate, so a spec that would otherwise be a generation-noop still pushes new limits. `structlog.info("risk_config_refreshed", ...)` fires exactly once per real change; a no-op refresh stays silent (audit-not-silent + no-log-spam). live vs paper cap floor is picked from `spec.lifecycle_state == "live"`. Four new tests grep-实存: `test_local_cap_refreshes_when_spec_changes_max_notional`, `test_fallback_breaker_refreshes_when_spec_changes_max_notional`, `test_risk_config_refresh_is_noop_when_unchanged`, `test_risk_config_refresh_uses_lifecycle_for_live_floor`.

**Verification snapshot**: `make verify` (--extra dev --extra nautilus) 288 passed, 1 warning at 04b-fix HEAD. Non-Custodial 4 red-line grep gates all clean (0.1 / 0.2 / 0.3 no hits; 0.4 only hits vendored `pandas_ta` OHLCV in `toolkit/`, which is third-party and outside the daemon money path).
