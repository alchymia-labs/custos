...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (T1-T5b historical slices landed; T5c pre-sign correction landed; T5d-A/T5d-B/T5e and protected T6-T9 open) | T5d-A consumes exact PS Plan 54 BOM/statement/detached-attestation and Crucible Plan 88 evidence/acceptance; T5d-B waits for the Crucible Plan 89 command producer | T5c -> PS54 -> CR88 -> T5d-A -> CR89 -> T5d-B/Plan19 T2 -> Plan19 T3-T5; no Speculum gate | Custos owns execution ABI, pre-sign ArtifactRef, toolkit, local verifier and its verification receipt. It never publishes the CR89 command schema or PS/Crucible business schemas. V1/V2 remain immutable historical evidence with no runtime fallback; the legacy PS `build-image.sh` -> Crucible Python lane remains independent compatibility only. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (19a T1 only until CR88/T5d-A/CR89 producer gates) | PRE-PLAN `324da6e`; CR88 evidence/acceptance -> Plan18 T5d-A -> CR89 command -> T5d-B/19T2; Plan 99 signed runner policy; Plan 18 staged/final BOM | T3/T4/T5 -> CR99/T7 -> T8a -> Plan 90A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance -> unchanged promotion/close-out | Reuse one SQLite deep module for desired/applied/outcomes/outbox/cap reservations. `deployment_instance_id` is the sole runtime stream identity; spec/digest/generation are signed fencing only. Old spec-keyed streams must drain and continue sequence explicitly. No second DB, outbox, journal or Plan 20. |
