...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (T1-T5b historical slices landed; T5c contract correction, protected T6 execution and T7-T9 open) | T6 local evidence remains valid only as local build/toolkit readiness; published v1/v2 ArtifactRef bytes are historical and cannot authorize production | T5c corrected pre-sign ArtifactRef -> PS Plan 54 BOM/statement/detached attestation -> Crucible Plan 88 native verification/acceptance -> protected T6 -> Plan 19 runtime; no Speculum gate | Custos owns execution ABI, pre-sign ArtifactRef, toolkit and local verifier. T5c removes bundle/policy self-reference additively without rewriting v1/v2; protected production remains fail closed until the corrected receipt lands. The legacy PS `build-image.sh` -> Crucible Python lane remains independent compatibility only. |
