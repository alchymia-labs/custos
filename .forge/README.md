...
| [18](plans/2026-07/18-typed-toolkit-strategy-contracts.md) | typed toolkit + strategy execution ABI + verified artifact consumer | ⏳ In progress (T5d-A `READY_CONTRACT_CONSUMER_ONLY`; T5d-B `READY_COMMAND_CONSUMER_CONTRACT_ONLY`; T5e and protected T6-T9 open) | Exact PS54/CR88 evidence assets and corrected current CR89 A2/B2 command assets are byte-vendored and pinned; old CR89 A/B are NON_CURRENT; one Custos consumer retains exact event bytes/fingerprint and publishes no command schema | T5c -> PS54 -> CR88 -> T5d-A -> CR89 -> T5d-B/Plan19 T2 -> Plan19 T3-T5; no Speculum gate | Custos owns execution ABI, ArtifactRefV2, ReceiptV2 and one CR-owned-command consumer only. Full BOM/evidence/acceptance bindings are contract-ready; production verifier/parser, runtime composition and production remain blocked. |
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2 contract-consumer STOP; T3-T10 open) | PRE-PLAN `324da6e`; CR88 evidence/acceptance -> T5d-A -> corrected CR89 A2/B2 -> T5d-B/19T2; Plan 99 signed runner policy; Plan 18 staged/final BOM | T3/T4/T5 -> CR99/T7 -> T8a -> Plan 90A -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance -> unchanged promotion/close-out | T2 adds no runtime wiring. Reuse one SQLite deep module for desired/applied/outcomes/outbox/cap reservations. `deployment_instance_id` remains the sole runtime stream identity; spec/digest/generation are signed fencing only. No second DB, outbox, journal or Plan 20. |
