...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + local runtime convergence | ⏳ In progress (19a T1 only until producer gates) | PRE-PLAN `324da6e`; Crucible Plan 89 command producer; Plan 99 runner-safety-policy-authority; Plan 18 staged/final BOM | 19a-c STOP → 19d-T8a candidate → Plan 90A compatibility → runtime RC/final-candidate → Plan 90B round trip → PS Plan 56 exact-candidate acceptance → unchanged promotion/close-out | Reuse one SQLite deep module for desired/applied/outcomes/outbox/cap reservations; T8a produces RunnerFact schema/golden/capability bytes before Plan 90A; Plan 90A gates RC only, while Plan 90B and PS 56 consume the exact final-candidate before Plan 19 completes. |
