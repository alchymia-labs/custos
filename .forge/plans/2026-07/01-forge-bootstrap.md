...
### DEVIATION: DEV-01-PYPROJECT-HATCH — pyproject.toml 缺 hatchling packages 配置
- **等级**: 低 (build 配置补丁, 无契约变更)
- **原因**: subtree split 时 `pyproject.toml` project.name 改为 `custos-runner`, 但 Python module 目录仍是 `src/arx_runner/`; hatchling 默认按 project.name 找目录 `src/custos_runner/`, 找不到即 build fail — `uv run pytest` 无法运行. Foundation Scan Gate 起草时未跑 `uv run pytest` 实证, 漏了这个基线陷阱 (lesson #14 空间维复发 — 现状扫忽略了 build tool 层)
- **影响**: `pyproject.toml` 加 `[tool.hatch.build.targets.wheel] packages = ["src/arx_runner"]` (5 行注释 + 1 行配置)
- **决定**: Task 5 前先补 pyproject.toml, 独立 commit `fix(custos): pyproject hatchling packages src/arx_runner (subtree split 遗留)`
- **更新的文档**: `pyproject.toml`; `.claude/rules/common-errors.md` 未来可追加 "hatchling build fail on rename" 条

### DEVIATION: DEV-01-WIRE-FIXTURES — test_wire_shapes.py 依赖 arx 仓库 fixture 路径
- **等级**: 中 (跨仓库路径依赖, 需 test baseline 分层)
- **原因**: `tests/test_wire_shapes.py` 硬编码 `tesseract-trading/backend/crates/telemetry/tests/wire_shapes/*.json` 路径 — 这是 arx 仓库内 fixture, subtree split 后 custos 独立仓库不含 fixture; 9 个 wire_shapes test 在 custos 独立场景必 fail. Foundation Scan 未 grep 硬编码路径
- **影响**: `make test` 跑 raw pytest 会看到 9 fail; `make verify` 需依赖 "可绿基线" (排除 wire_shapes 目录) 才有意义
- **决定**: Makefile 分层 target:
  - `make test` = `uv run pytest tests/` (raw, 反映现实, 允许 9 已知 fail)
  - `make test-baseline` = `uv run pytest tests/ --ignore=tests/test_wire_shapes.py` (当前可绿基线, 115 pass)
  - `make verify` = `check + test-baseline` (发布门用可绿基线)
  - 偏离原因写进 Makefile 目标注释 + README §Not Included Yet 追加一条 "wire fixture 独立化"
- **更新的文档**: `Makefile` (Task 5) + `README.md` (未来 plan) + `docs/guides/04-testing.md` (Task 7 说明 wire_shapes 分层)
...
- **完成日期**: 2026-07-07
- **总 Task 数**: 8 (全部 ✅)
- **偏离数**: 3 (bootstrap 后置 + DEV-01-PYPROJECT-HATCH + DEV-01-WIRE-FIXTURES)
- **验证结果**: 全部通过 (`make verify` = check + test-baseline 115 pass)
- **实施 commit 范围**: `4384bff..HEAD` (11 commits)
- **契约影响**:
  - 新建独立规则集 `.claude/rules/*.md` (9 file, 自足于 workspace)
  - 新建 `.gitignore` (Python + non-custodial vault + examples env)
  - 新建 `Makefile` (10 target, line-length=100)
  - 新建 `CLAUDE.md` refine 为完整导航图 (143 行)
  - 新建 `.forge/README.md` plan 索引
  - 迁 6 module docs 到 `docs/design/` + 补 4 骨架 (00-overview / 01-architecture / 02-module-design / 03-implementation)
  - 补 `docs/ops/` (05-deployment + runbook) 和 `docs/guides/` (04-testing + dev-guide)
  - 修 `pyproject.toml` 加 hatchling packages + ruff config + ruff dev extra
  - 一次性 ruff format sweep (16 file, 无功能变更)
- **红线守护 (Non-Custodial 4 红线 grep 记录)**:
  - 红线 0.1 (Key 不出进程): `grep -rnE 'log\.(info|debug|warning).*api[_-]?key' src/ tests/` — 无命中 ✅
  - 红线 0.2 (G6 gate): 仅 `nautilus_host.py` 内引用 CEX/NT client, 无绕过 ✅
  - 红线 0.3 (失联≠停止): `grep -rn 'stop_all_strategies\|force_shutdown' src/arx_runner/reconcile.py` — 无命中 ✅
  - 红线 0.4 (Decimal): `grep -rnE 'float\(.*price|float\(.*amount' src/` — 无命中 ✅
- **失败模式覆盖**: 本 plan 是纯基础设施, 不引入 runtime 失败模式; 测试基线仍 115 pass 无回归
- **lesson #11 元层复发发现**: Task 8 集成验证 grep 实证 `SubjectBuilder` 类不存在
  (实际是 `build_subject()` 函数), 已修正 4 处文档引用 (docs/design 3 file + rules 1 file);
  证明 `code-reviewing/SKILL.md` "已文件引用符号必 grep 实证" 门禁生效
- **遗留项**:
  - `test_wire_shapes.py` 9 fail (DEV-01-WIRE-FIXTURES) 待 Plan 02+ 独立 fixture 生成机制
  - `pyright` 类型检查 target 待 Plan 02+
  - `nt-runtime` extra 待 Plan 00a
  - Docker/example 待 Plan 00c
  - Python 模块 rename `arx_runner` → `custos_runner` 待独立 plan (README 已声明)
