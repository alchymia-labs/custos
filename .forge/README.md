...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B daemon boundary CODE-ONLY; T7C reopened for per-mode SIM/LIVE transport correction; T8a candidate + T8b Phase-A READY; T9-T10 open) | T7B schema v4, native client facade and daemon composition share the RunnerFact store and fail closed without owner policy. T7C must replace the provisional multi-mode credential/durable shape with one supervisor composing independent per-mode vaults, JWTs, connections, consumers, publishers and readiness across isolated SIM/LIVE transport domains. | CR99 publication/consumption -> CR100 corrected T1 and real runtime evidence -> Custos per-mode consumer receipt -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Phase-A compatibility remains ready, but T7C code and receipts are reopened; real owner policy consumption, production transport evidence, exact runtime BOM/RC, Phase B, live, runtime and production readiness remain false. |
