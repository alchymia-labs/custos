...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + local runtime convergence | ⏳ In progress (T1 only until producer gates) | PRE-PLAN `324da6e`; clean Crucible command producer; signed runner-level cap policy | Custos 0.4.0; PS Plan 56 acceptance | Reuse the existing RunnerFact SQLite outbox and commit applied state + lifecycle fact in one transaction keyed by `deployment_instance_id`; preserve the engine protocol, add readiness/quarantine, consume producer-owned schemas/policies, and keep live fail closed until cap enforcement is non-bypassable. |
