...
| 实体 | 关键字段 | 不变量 |
|------|---------|-------|
| **DeploymentSpec**（本地缓存的云端权威）| `spec_id` · `generation`(从 1 开始、单调递增) · `trading_mode`(sandbox/testnet/live) · `lifecycle_state`(running/paused/stopped/archived) · `strategy_path` · `provenance_ref.credential_id` · `connector` · `pairs` · `leverage` · `strategy_config`(opaque dict) · `strategy_registry_name`(optional) · `code_hash`(optional; live 必须为 64 位小写 hex) · `sandbox.starting_balances` · `approved_by` · `risk_config` · `nautilus_config` | `custos.contracts.DeploymentSpec` 是 normative consumer model，未知字段拒绝；reconciler 在进入 G6/host 前先 `model_validate()`；sandbox 必须给 starting balances；live 必须给 strategy directory hash；`strategy_config` 原样传入 strategy factory；`strategy_registry_name` 若给出，loader 内省断言 name→class 绑定一致；`lifecycle_state=paused` 时 zombie watchdog 不升级 degraded |
