...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B CODE-ONLY; T7C CR100 transport planned; T8a producer candidate READY; T8b-T10 open) | T7C must replace anonymous/plaintext wildcard NATS with local User NKey custody, Crucible-issued JWT, pinned TLS, exact tenant+runner durable and reconnect-denial evidence; T8a immutable producer remains contract-only | CR100 T1-T7 receipts -> T7C transport consumer -> Plan 90A/T8b -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Code-only transport consumption may proceed, but production credential/durable, CR90 compatibility, engine/daemon, runtime RC, live, runtime and production readiness remain false. |
