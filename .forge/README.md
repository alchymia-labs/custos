...
| [19](plans/2026-07/19-crucible-command-runner-fact-runtime-convergence.md) | Crucible command + single RunnerFact state store + production runtime convergence | ⏳ In progress (T2-T4 READY scopes; T5 PREPARED-BLOCKED; T6 READY; T7A READY; T7B daemon boundary CODE-ONLY; T7C authenticated transport CODE-ONLY; T8a candidate + T8b Phase-A READY; T9-T10 open) | T7B schema v4, native client facade and daemon composition now share the RunnerFact outbox/store and fail closed without a durable owner policy. Actual CR99 policy publication/consumption remains open. T7C production durable plus reconnect-denial attestation also remains open | CR99 publication/consumption -> CR100 T8 runtime evidence -> locked runtime RC -> Plan 90B -> PS56 exact-candidate acceptance | Phase-A compatibility is consumed, but real owner policy consumption, production transport evidence, exact runtime BOM/RC, Phase B, live, runtime and production readiness remain false. |
