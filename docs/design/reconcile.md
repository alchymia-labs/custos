...
| `NautilusHostProtocol` | `async deploy(spec, credential) -> str` / `async reconfigure(spec)` / `async stop(spec_id)` / `supports_live() -> bool` / `supports_venue(venue) -> bool`（后二为 sync capability） | reconciler 依赖的 NT host 契约（duck-typed）；capability 方法供 G6 gate 查询。host 由入口 `--use-nt-host` 选择（默认 NoopHost，见 [nautilus_host.md §CLI 入口](nautilus_host.md)） |
