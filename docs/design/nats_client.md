...
`tenant_id`、`strategy_id` 与 `DeploymentSpec.spec_id` 统一匹配
`^[a-zA-Z0-9_-]{1,64}$`；其中 `spec_id` 会成为
`deployment_status.<runner_id>.<spec_id>` 的 subject token，必须在进入 reconciler 前拒绝
点号、路径分隔符、控制字符和非 ASCII 文本，避免 token 注入与 status subject 漂移。

0.3.0 producer 通过公共 CLI 生成同一 wire，而不是复制 subject/envelope 规则：
...
命令先用 `DeploymentSpec` 做 runtime validation，再用 `DeploymentMessage.create()` 发布并
等待 JetStream acknowledgement。下游只组装策略代码与 opaque `strategy_config`；它不拥有
Custos stream topology、transport wrapper 或派生 runtime image。
