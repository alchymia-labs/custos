...
| 实体 | 关键字段 | 不变量 |
|------|---------|-------|
| **DeploymentSpec**（本地缓存的云端权威）| `spec_id` · `tenant_id` · `strategy_id` · `version` · `parameters`(JSON) · `execution_engine_binding_id` · `trading_mode`(testnet/sandbox/live) · `target_runner_id`（必等于本机 `runner_id`）· `generation`(单调递增) · `code_hash`(live 必持 provenance) · `pulled_at` · `strategy_registry_name`(optional) · `nautilus_config`(optional dict) | 只读（云端权威）；`target_runner_id ≠ 本机 runner_id` 时忽略；`trading_mode=live` 时 `code_hash` 必与本地 image sha 匹配；`strategy_registry_name` 若给出, 加载后经 ps toolkit registry 内省断言 name→class 绑定与 loader 找到的类一致（防 heuristic 静默错加载）；`nautilus_config` 若给出, 支持 `timeout_{connection,reconciliation,portfolio,disconnection}` + `reconciliation_lookback_mins` 覆盖 NT 内部默认 |
